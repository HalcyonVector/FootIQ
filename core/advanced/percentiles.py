...
from core.advanced.metrics_master import CATEGORIES, CATEGORY_ORDER
...
def _percentile_rank(value: float, series: pd.Series) -> int:
    valid = series.dropna()
    if len(valid) < 5 or value is None:
        return 0
    return int(round((valid < value).mean() * 100))
...
def _build_final_third_stats(row: dict, cohort: pd.DataFrame) -> list[dict]:
    ...
    have_data = row.get("ft_touches_p90") and len(cohort) >= 5
    if have_data:
...
def build_all_categories(row: dict, position: str, season: str, df: pd.DataFrame) -> list[dict]:
    ...
    Returns a list of {key, label, icon, rows: [{player_name, team, photo, stats}]}
    ready for the frontend's renderPctTable, one entry per category (GK-gated)."""
    ...
    for cat_key in CATEGORY_ORDER:
        ...
        if cat["gk_only"] and not is_gk:
            continue
        if not cat["gk_only"] and is_gk:
            continue  # keepers only see the Goalkeeping tab, not outfield categories
        ...
        if cat_key == "final_third":
            ...
            for col, label, unit in cat["metrics"]:
                ...
                if value is None or (isinstance(value, float) and pd.isna(value)):
                    ...
                    continue
                pct = _percentile_rank(value, cohort[col]) if col in cohort.columns else 0
